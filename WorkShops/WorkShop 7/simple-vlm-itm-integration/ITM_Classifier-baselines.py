...
class ITM_Dataset(Dataset):
    ...
    def load_data(self):
        ...
        with open(self.data_file) as f:
            ...
            for line in lines:
                ...
                # get preditions from VLM
                self.get_predictions_from_VLM(img_path, question_text, answer_text)
    ...
    def get_predictions_from_VLM(self, img_path, question_text, option_text):
        print("img_path=",img_path)
        print("question_text=",question_text)
        print("answer_text=",option_text)
        ...
        # Decode answer
        vqa_answer = processor.decode(output[0], skip_special_tokens=True)
        vqa_answer_embedding = model_ss.encode(vqa_answer, convert_to_tensor=True)

        # (2) Obtain a matching score based on the similarity between ground truth option and predicted answer
        option_embedding = model_ss.encode(option_text, convert_to_tensor=True)
        score = util.pytorch_cos_sim(vqa_answer_embedding, option_embedding).cpu().item()
        print(f"Option: {option_text} Predicted Answer: {vqa_answer} Similarity_Score: {score}")
